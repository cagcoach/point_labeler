...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r10,r11,t0,t1) reduction(+:mum0,mum1, mut0,mut1) // schedule (dynamic,2)
    for (i=0; i<nact; i++) {
      ...
      query[0] = (float)(r00*T[idx2+0] + r01*T[idx2+1] + t0);
      query[1] = (float)(r10*T[idx2+0] + r11*T[idx2+1] + t1);
...
//#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
    ...
    for (i=0; i<nact; i++) {
      ...
      int32_t idx3 = 3*active[i];

      // transform point according to R|t
      query[0] = (float)(r00*T[idx3+0] + r01*T[idx3+1] + r02*T[idx3+2] + t0);
      query[1] = (float)(r10*T[idx3+0] + r11*T[idx3+1] + r12*T[idx3+2] + t1);
      query[2] = (float)(r20*T[idx3+0] + r21*T[idx3+1] + r22*T[idx3+2] + t2);
      ...
      auto pmvali = p_m.val[i];
      auto Mtreethedataresult0idx = M_tree->the_data[result[0].idx];
      pmvali[0] = Mtreethedataresult0idx[0]; mum0 += pmvali[0];
      pmvali[1] = Mtreethedataresult0idx[1]; mum1 += pmvali[1];
      pmvali[2] = Mtreethedataresult0idx[2]; mum2 += pmvali[2];

      // set template point

      auto ptvali = p_t.val[i];
      ptvali[0] = query[0]; mut0 += ptvali[0];
      ptvali[1] = query[1]; mut1 += ptvali[1];
      ptvali[2] = query[2]; mut2 += ptvali[2];
      //});
    }
    mu_m.val[0][0] = mum0;
    mu_m.val[0][1] = mum1;
  ...
  //std::cout<<(R_-Matrix::eye(3)).l2norm()<<" | "<< t_.l2norm()<<std::endl;
